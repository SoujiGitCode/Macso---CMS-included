...
class FrontendController extends Controller {
    ...
    public function index() {
        try {
            ...
            $url_en = 'en';
            $url_es = 'es';
            $sales = Properties::orderBy('created_at', 'desc')->where('type_id','2')->offset(0)->limit(6)->get();
            $rental = Properties::orderBy('created_at', 'desc')->where('type_id','1')->offset(0)->limit(6)->get();

            return view('frontend.home', ['subtitle' => $subtitle, 'sales' => $sales,'rental'=>$rental, 'lang' => $lang, 'url_en' => $url_en, 'url_es' => $url_es]);
        } catch (Exception $ex) {
            return false;
        }
        return false;
    ...
    /**
     * Display Sales- Houses
     * @return type
     */
    public function salesHouses() {
        try {
            $url = explode("/", $_SERVER["REQUEST_URI"]);
            if ($url[1] != 'en' || $url[1] == 'es') {
                $lang = 'es';
                $subtitle = " - venta-casas";
                $this->visits('venta-casas');
            } else {
                $lang = 'en';
                $subtitle = "- sale-houses";
                $this->visits('sale-houses');
            }
            $url_en = 'en/sale-houses';
            $url_es = 'venta-casas';
            return view('frontend.sale', ['subtitle' => $subtitle, 'lang' => $lang, 'url_en' => $url_en, 'url_es' => $url_es]);
        } catch (Exception $ex) {
            return false;
